...
# ------------------------------------------------------------------------------
# Prompt compatibility helpers (new SleepForge prompt keys + old prompt keys)
# ------------------------------------------------------------------------------

def _get_prompt(prompts, new_name: str, old_name: str = None):
    """Return prompt template by new key first, then old key."""
    if hasattr(prompts, new_name):
        return getattr(prompts, new_name)
    if old_name and hasattr(prompts, old_name):
        return getattr(prompts, old_name)
    raise AttributeError(f"Missing prompt template: {new_name}" + (f" / {old_name}" if old_name else ""))


def _safe_format(template: str, **kwargs):
    """Format with provided variables; keep missing placeholders unchanged instead of crashing."""
    try:
...
def _parse_box_action(text: str, allowed):
    """Parse \\box{{Action}} / \\box{Action} or Action: ... / Command: ... ."""
    if not text:
        return None
    allowed_upper = {a.upper(): a.upper() for a in allowed}
    m = re.search(r"\\box\{+\s*([A-Za-z_+-]+)\s*\}+", text, re.IGNORECASE)
    if m:
        act = m.group(1).strip().upper()
        if act in allowed_upper:
            return allowed_upper[act]
    m = re.search(r"(?:Action|Command)\s*:\s*([A-Za-z_+-]+)", text, re.IGNORECASE)
    if m:
        act = m.group(1).strip().upper()
        if act in allowed_upper:
            return allowed_upper[act]
...
def _parse_guidance(text: str, default: str = "No specific guidance provided."):
    """Parse \\guidance{{...}}, \\gradient{{...}}, \\advice{{...}}, or line-based guidance."""
    if not text:
        return default
    for key in ["guidance", "gradient", "advice", "diagnosis"]:
        m = re.search(rf"\\{key}\{{+\s*(.*?)\s*\}}+", text, re.DOTALL | re.IGNORECASE)
        if m:
            val = m.group(1).strip()
            if val:
                return val
    m = re.search(r"(?:Guidance|Advice|Gradient)\s*:\s*(.*)", text, re.DOTALL | re.IGNORECASE)
    if m:
        val = m.group(1).strip()
        if val:
            return val
...
class TextGradOptimizer:
    def __init__(self, cfg, memories, memory_stats, log_path):
        ...
        self.action_re = re.compile(
            r'(?:\\box\{+\s*|Action:\s*)(REPAIR|REFINE|EXPAND|REPLACE|CREATE)',
            re.IGNORECASE,
        )
        
        
        
        self.advice_re = re.compile(
            r'(?:\\(?:guidance|advice|gradient)\{+\s*|(?:Guidance|Advice|Gradient):\s*)(.*?)(?:\}+\s*$|(?=\n[A-Z][A-Za-z ]*:)|$)',
    ...
    def _batch_diagnose(self, tasks: List[OptimizationTask]):
        ...
        for task, out in zip(tasks, outputs):
            ...
            parsed_action = m_act.group(1).upper() if m_act else "REPAIR"
            # New paper prompt uses Repair; old implementation path for rewriting is REFINE.
            if parsed_action == "REPAIR":
                parsed_action = "REFINE"
            task.expert_action = parsed_action
            
            
            gradient = _parse_guidance(out, default=out.strip())
    ...
    def _batch_execute_and_expand(self, tasks: List[OptimizationTask]) -> List[OptimizationTask]:
        ...
        for task in tasks:
            ...
            if task.expert_action == "EXPAND":
                ...
                tpl = _get_prompt(self.cfg.optimizer.prompts, "repair_prompt", "appgrad_low_refine")
                p_old = _safe_format(
                    tpl,
                    content=task.original_content,
                    gradient=f"Keep the useful core, but distinguish it from the new concept. Guidance: {gradient}",
                    guidance=f"Keep the useful core, but distinguish it from the new concept. Guidance: {gradient}",
                )
                ...
                tpl = _get_prompt(self.cfg.optimizer.prompts, "replace_prompt", "appgrad_low_replace")
                p_new = _safe_format(
                    tpl,
                    neg_queries=neg_text,
                    gradient=f"Create a new memory specific to these queries. Guidance: {gradient}",
                    guidance=f"Create a new memory specific to these queries. Guidance: {gradient}",
                )
                ...
                tpl = _get_prompt(self.cfg.optimizer.prompts, "replace_prompt", "appgrad_low_replace")
                p = _safe_format(tpl, neg_queries=neg_text, gradient=gradient, guidance=gradient)
                ...
                tpl = _get_prompt(self.cfg.optimizer.prompts, "repair_prompt", "appgrad_low_refine")
                p = _safe_format(tpl, content=task.original_content, gradient=gradient, guidance=gradient)
                task.student_prompt = p
                prompts.append(p)
                active_tasks.append(task)
    ...
    def _batch_evaluate_loop(self, tasks: List[OptimizationTask]):
        ...
        for retry_idx in range(self.max_retries + 1):
            ...
            for t in pending_tasks:
                ...
                tpl = _get_prompt(self.cfg.optimizer.prompts, "skill_memory_audit", "expert_judge")
                p = _safe_format(tpl, failed=neg_q, old=t.original_content, new=t.generated_content)
            ...
            for t, out in zip(pending_tasks, judge_outs):
                ...
                if verdict == "PASS":
                    ...
                    if retry_idx < self.max_retries:
                        ...
                        tpl = _get_prompt(self.cfg.optimizer.prompts, "rollback_retry", "retry_prompt")
                        new_prompt = _safe_format(
                            tpl,
                            ori=t.student_prompt,
                            failed=neg_q,
                            bad=t.generated_content,
                            feedback=feedback,
                        )
