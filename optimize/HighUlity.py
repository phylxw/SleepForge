...
def _evolve_acceptance_batch(cfg, items):
    prompts = []
    for it in items:
        tpl = _get_prompt(cfg.optimizer.prompts, "skill_memory_audit", "expert_judge")
        prompts.append(_safe_format(
            tpl,
...
def evolve_high_score_opt(cfg: DictConfig, memories: Dict, memory_stats: Dict,log_file_path, high_ids: List[str]) -> Set[str]:
    ...
    for i in range(0, len(target_ids), batch_size):
        ...
        for mid in chunk_ids:
            ...
            try:
                tpl = _get_prompt(cfg.optimizer.prompts, "high_utility_refinement", "high_grad_expert")
                prompt = _safe_format(tpl, content=base_text, neg_queries=neg_text)
        ...
        for meta, expert_resp in zip(chunk_metadata, expert_outputs):
            ...
            raw_action = _parse_box_action(
                expert_resp,
                allowed=["IGNORE", "SUPPLEMENT", "SPLIT", "REFINE"]
            ) or "IGNORE"
            # New paper prompt uses Refine; old implementation path is SUPPLEMENT.
            action = "SUPPLEMENT" if raw_action == "REFINE" else raw_action

            advice = _parse_guidance(expert_resp)
            gradient = advice
            ...
            if action == "SUPPLEMENT":
                tpl = _get_prompt(cfg.optimizer.prompts, "refine_prompt", "appgrad_high_supplement")
                s_prompt = _safe_format(
                    tpl,
                    original_content=meta["base_text"],
                    advice=advice,
                    guidance=advice,
                )
                ...
                tpl = _get_prompt(cfg.optimizer.prompts, "split_prompt", "appgrad_high_split")
                s_prompt = _safe_format(
                    tpl,
                    neg_text=meta["neg_text"],
                    neg_queries=meta["neg_text"],
                    advice=advice,
                    guidance=advice,
                    num=1,
                )
        ...
        for round_idx in range(max_retries + 1):
            ...
            for i, cand in enumerate(candidates):
                if cand["status"] == "PENDING":
                    ...
                    tpl = _get_prompt(cfg.optimizer.prompts, "rollback_retry", "retry_prompt")
                    retry_prompt = _safe_format(
                            tpl,
                            failed=neg_q,
                            ori=base_prompt,
                            bad=(prev_out or ""),
